...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, custom_pipeline=None):
        ...
        self.best_loss_step: Optional[int] = None
    ...
    def _load_best_model_info(self):
        ...
        try:
            with open(self.best_model_info_path, 'r', encoding='utf-8') as f:
                best_model_info = json.load(f)
        except Exception as e:
            print_acc(f"Warning: Could not load best model info from {self.best_model_info_path}: {e}")
            return

        loss = self._loss_value_to_float(best_model_info.get('loss'))
        if loss is not None:
            self.best_loss = loss

        step = best_model_info.get('step')
    ...
    def _get_primary_loss_value(self, loss_dict):
        if loss_dict is None:
            return None
        if 'loss' in loss_dict:
            return self._loss_value_to_float(loss_dict['loss'])

        loss_values = []
        for key, value in loss_dict.items():
            if key.startswith('loss'):
                value = self._loss_value_to_float(value)
    ...
    def maybe_save_best_model(self, loss_dict):
        ...
        if self.best_loss is not None and current_loss >= self.best_loss:
            return

        self.best_loss = current_loss
        self.best_loss_step = self.step_num

        if self.progress_bar is not None:
            self.progress_bar.pause()

        try:
            print_acc(f"\nNew best loss {current_loss:.6f} at step {self.step_num}, saving best checkpoint")
            self.save(
    ...
    def save(self, step=None, save_dir=None, name_suffix='', save_optimizer=True, cleanup_old=True):
        if not self.accelerator.is_main_process:
            return
        flush()
        if self.ema is not None:
        ...
        filename = f'{self.job.name}{step_num}{name_suffix}.safetensors'
        ...
        if not self.is_fine_tuning:
            ...
            if self.embedding is not None:
                emb_filename = f'{self.embed_config.trigger}{step_num}{name_suffix}.safetensors'
                emb_file_path = os.path.join(save_dir, emb_filename)
            ...
            if self.decorator is not None:
                dec_filename = f'{self.job.name}{step_num}{name_suffix}.safetensors'
                dec_file_path = os.path.join(save_dir, dec_filename)
            ...
            if self.adapter is not None and self.adapter_config.train:
                ...
                filename = f'{adapter_name}{step_num}{name_suffix}.safetensors'
                file_path = os.path.join(save_dir, filename)
                # save adapter
            ...
            if self.sd.refiner_unet and self.train_config.train_refiner:
                ...
                filename = f'{refiner_name}{step_num}{name_suffix}.safetensors'
                file_path = os.path.join(save_dir, filename)
        ...
        if self.snr_gos:
            ...
            path_to_save = os.path.join(save_dir, 'learnable_snr.json')
            with open(path_to_save, 'w') as f:
                json.dump(json_data, f, indent=4)
        
        print_acc(f"Saved checkpoint to {saved_model_path}")

        # save optimizer
        if save_optimizer and self.optimizer is not None:
            try:
                filename = f'optimizer.pt'
                optimizer_file_path = os.path.join(save_dir, filename)
                try:
        ...
        if cleanup_old and save_dir == self.save_root:
            self.clean_up_saves()
        self.post_save_hook(saved_model_path)
    ...
    def run(self):
        ...
        for step in range(start_step_num, self.train_config.steps):
            ...
            with torch.no_grad():
                ...
                self.maybe_save_best_model(loss_dict)
