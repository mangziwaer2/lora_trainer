...
class StableDiffusion:
    ...
    def generate_images(
        ...
        max_sample_pixels = max([cfg.width * cfg.height for cfg in image_configs], default=0)
        total_vram = None
        if torch.cuda.is_available() and self.device_torch.type == "cuda":
            try:
                total_vram = torch.cuda.get_device_properties(self.device_torch).total_memory
            except Exception:
                total_vram = None
        use_low_vram_sampling = self.low_vram or (
            total_vram is not None
            and total_vram <= (8 * 1024 * 1024 * 1024)
            and max_sample_pixels >= (512 * 512)
        )
        ...
        if pipeline is None:
            ...
            if use_low_vram_sampling:
                ...
                try:
                    ...
                    pass
                try:
                    if max_sample_pixels >= (1024 * 1024):
        ...
        if self.refiner_unet:
            ...
            if use_low_vram_sampling:
                try:
                    refiner_pipeline.enable_attention_slicing("auto")
                except Exception:
                    pass
                try:
